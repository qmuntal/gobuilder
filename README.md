...
The `builder` workflow follows the Go dashboard builder setup by minting a LUCI machine token with `luci_machine_tokend`, downloading `swarming_bot.zip` from LUCI Swarming, and starting the Swarming bot. Before it can work, the builder must be approved and defined by the Go team in LUCI.

The `builder` workflow runs on a Windows ARM64 runner. It downloads `luci_machine_tokend.exe` from `go-builder-data`, matching the Azure Windows ARM64 setup in `golang/build`. The Swarming bot handles CIPD-managed payloads after it starts.

The certificate and private key are only used while minting `token.json`; the workflow deletes those PEM files before downloading and starting the Swarming bot. The downloaded bot is patched locally with an `on_after_task` hook that posts a shutdown event and exits the bot process after the first completed task, making each GitHub Actions builder run a one-task bot.
...
`luci_machine_tokend` receives `-ts-mon-autogen-hostname` and `-ts-mon-task-hostname` so LUCI monitoring treats the logical bot hostname as autogenerated. This is a monitoring hint; the Swarming bot identity still comes from `SWARMING_BOT_ID`.
